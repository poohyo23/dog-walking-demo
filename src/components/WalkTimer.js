...
class WalkTimer extends React.Component{
    ...
    render() {
        return<div id="container">
            ...
            <div className="timer">
              <h2>{this.state.currentTime}초</h2>
              {this.state.isChecked ? 
              <button className="walk_end" onClick={this.timeOnOff}>산책 끝!</button> :
              <button className="walk_start" onClick={this.timeOnOff}>
                {this.state.isGoing ? "산책 그만" : "산책 시작"}
                </button>}
              
            </div>

            {this.state.isChecked ? <div className="walk_after">
              <h2>오늘도 즐거운 산책이었나요?</h2>
              <h4>산책 후 마무리까지 완벽하게 체크해보세요!</h4>
              <ul>
                <li>댕댕이와 매너있는 산책을 위해 배변봉투는 길거리에 버리지 마시고 <mark>집에 챙겨가기!</mark></li>
                <li>털에 묻은 먼지는 <mark>털어주고</mark> 발바닥 닦고 잘 <mark>건조시켜주기!</mark></li>
                <li>산책 후 충분한 <mark>수분보충</mark>과 간단한 <mark>간식</mark>으로 체력관리</li>
              </ul>
